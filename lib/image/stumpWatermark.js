...
function buildWatermarkCommand(imagefile, watermark) {
  if (!imagefile || !watermark) {
    trace.info(`Watermark required parameters missing ${imagefile}...`);
    return null;
  }

  const { mark, text } = watermark;
  if (!mark && !text) {
    trace.info(`Watermark required information missing ${imagefile}...`);
    return null;
  }
...
function stumpWatermark(imagefile, watermark) {
  return new Promise((resolve, reject) => {
    ...
    trace.info(`Stumping watermark on ${imagefile}...`);
    ...
    shelljs.exec(command, (code, stdout, stderr) => {
      ...
      trace.info(`Watermark process success: ${imagefile} (${stdout})`);
      resolve(0);
    });
  });
}
...
async function stumpMultipleWatermarks(fileimage, watermarks) {
  ...
  for (const watermark of watermarks) {
    const error = await stumpWatermark(fileimage, watermark);
    if (error) {
      return error;
    }
  }
