...
describe('savePicTags', () => {
  ...
  it('should return an error when picFilename not exists', async () => {
    const response = await savePicInfo('./nonExistentFile.jpg');
    expect(response.error).toBeDefined();
    expect(response.error.toString()).toEqual(
      'Error: Could not found the file ./nonExistentFile.jpg',
    );
  });
